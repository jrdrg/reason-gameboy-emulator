...
function ld_hl_nn(s) {
  var match = ld_n_nn(s);
  return Cpu_types$GameboyEmulator.newState(Cpu$GameboyEmulator.wL(match[1], Cpu$GameboyEmulator.wH(match[0], match[3])), Caml_option.some(match[2]), undefined, s);
}
...
function jr_nz_n(s) {
  ...
  var match$1 = Cpu$GameboyEmulator.Flags.isSet(/* Z */0, cpu[/* registers */3][/* f */7]) ? /* tuple */[
      ...
      (pc + 1 | 0) + Cpu$GameboyEmulator.signed(n) | 0,
      3
    ];
  var pc$1 = match$1[0];
  ...
  return Cpu_types$GameboyEmulator.newState(Cpu$GameboyEmulator.machineCycles(match$1[1], Cpu$GameboyEmulator.setPc(pc$1, cpu)), Caml_option.some(match[1]), undefined, s);
}
